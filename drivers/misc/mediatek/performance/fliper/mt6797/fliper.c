...
#endif
	if (is_sport)
		cg_set_threshold(CG_DEFAULT_SPORT_LPM, CG_DEFAULT_SPORT_HPM);
	else
		cg_set_threshold(CG_DEFAULT_NORMAL_LPM, CG_DEFAULT_NORMAL_HPM);
	return 0;
}
		...
		size_t cnt, loff_t *data)
{
	...
	if (strncmp(option, "ENABLE_CG", 9) == 0) {
		...
		if (fliper_debug >= 2)
			...
			disable_fliper_polling();
	} else if (strncmp(option, "SPORT", 5) == 0) {
		is_sport = arg1;
		cg_restore_threshold();
	} else {
