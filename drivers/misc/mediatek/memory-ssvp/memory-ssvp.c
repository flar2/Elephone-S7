...
static bool no_release_memory = true;
#else
static bool no_release_memory;
#endif
...
static int set_pmd_mapping(unsigned long start, phys_addr_t size, int map)
{
	...
	if ((start != (start & PMD_MASK))
			...
			|| !size || !start) {
		pr_alert("[invalid parameter]: start=0x%lx, size=0x%p\n",
			start, (void *)size);
		return -1;
	}

	pr_debug("start=0x%lx, size=0x%p, address=0x%p, map=%d\n",
				start, (void *)size, (void *)address, map);
	...
	return 0;
fail:
	pr_alert("start=0x%lx, size=0x%p, address=0x%p, map=%d\n",
				start, (void *)size, (void *)address, map);
...
int tui_region_offline(phys_addr_t *pa, unsigned long *size)
{
	...
	if (_svpregs[SSVP_SUB_TUI].state == SVP_STATE_ON) {
		...
		if (no_release_memory)
			...
			page = cma_alloc(
					ssvp_cma.cma,
					_svpregs[SSVP_SUB_TUI].count, 10);
			if (page) {
...
static int __svp_region_online(void)
{
	...
	if (_svpregs[SSVP_SUB_SVP].state == SVP_STATE_ONING_WAIT) {
		...
		if (_svpregs[SSVP_SUB_SVP].is_unmapping)
			ret_map = pmd_mapping((unsigned long)__va((page_to_phys(_svpregs[SSVP_SUB_SVP].page))),
					_svpregs[SSVP_SUB_SVP].count << PAGE_SHIFT);

		if (ret_map < 0) {
			pr_alert("[remapping fail]: virt:0x%lx, size:0x%lx",
					(unsigned long)__va((page_to_phys(_svpregs[SSVP_SUB_SVP].page))),
					_svpregs[SSVP_SUB_SVP].count << PAGE_SHIFT);
			...
			wrap_cma_pages = _svpregs[SSVP_SUB_SVP].page;
			goto online_done;
		}

		_svpregs[SSVP_SUB_SVP].is_unmapping = false;
		cma_release(ssvp_cma.cma, _svpregs[SSVP_SUB_SVP].page, _svpregs[SSVP_SUB_SVP].count);
		svp_usage_count -= _svpregs[SSVP_SUB_SVP].count;
	} else {
		retval = -EBUSY;
		goto out;
...
int svp_region_offline(phys_addr_t *pa, unsigned long *size)
{
	...
	if (_svpregs[SSVP_SUB_SVP].state == SVP_STATE_ON) {
		...
		if (no_release_memory)
			...
			shrink_ion_by_scenario();
			page = cma_alloc(ssvp_cma.cma, _svpregs[SSVP_SUB_SVP].count, 10);
			...
			if (page) {
				...
				if (ret_map < 0) {
					...
					_svpregs[SSVP_SUB_SVP].is_unmapping = true;
			}
		}
		...
		if (page) {
			...
			/* call secmem_enable */
			res = secmem_api_enable(page_to_phys(page),
					_svpregs[SSVP_SUB_SVP].count << PAGE_SHIFT);
...
static int __init memory_ssvp_debug_init(void)
{
	...
	if (no_release_memory) {
		wrap_cma_pages = cma_alloc(ssvp_cma.cma, ssvp_cma.count, 10);
