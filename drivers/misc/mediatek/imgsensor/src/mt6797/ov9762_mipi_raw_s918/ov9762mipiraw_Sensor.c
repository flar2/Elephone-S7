...
#define LOG_1 printk("OV976,MIPI 1LANE\n")
#define LOG_2 printk("preview 1470*1100@30fps,256Mbps/lane\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.linelength = 1366,				//record different mode's linelength
		.framelength = 1024,			//record different mode's framelength
...
{{ 1280, 960,	  0,  0, 1280, 960, 1280, 960, 0, 0, 1280, 960,	  0,	0, 1280, 960}, // Preview 
 { 1280, 960,	  0,  0, 1280, 960, 1280, 960, 0, 0, 1280, 960,	  0,	0, 1280, 960}, // capture 
 { 1280, 960,	  0,  0, 1280, 960, 1280, 960, 0, 0, 1280, 960,	  0,	0, 1280, 960}, // video 
 { 1280, 960,	  0,  0, 1280, 960, 1280, 960, 0, 0, 1280, 960,	  0,	0, 1280, 960}, // //hight speed video 
 { 1280, 960,	  0,  0, 1280, 960, 1280, 960, 0, 0, 1280, 960,	  0,	0, 1280, 960}};// slim video 
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3091, 0x1c);
	write_cmos_sensor(0x3092, 0x04);
	write_cmos_sensor(0x3093, 0x00);
	write_cmos_sensor(0x3094, 0x00);
	write_cmos_sensor(0x30b3, 0x1f);
	write_cmos_sensor(0x30b4, 0x02);
	write_cmos_sensor(0x30b1, 0x01);
	write_cmos_sensor(0x30b0, 0x0a);
	write_cmos_sensor(0x034c, 0x05);
	write_cmos_sensor(0x034d, 0x00);
	write_cmos_sensor(0x034e, 0x03);
	write_cmos_sensor(0x034f, 0xc0);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
	printk("[OV9760MIPI] exit OV9760MIPISetTestPatternMode function \n");
    return ERROR_NONE;
}
		...
		do {
            *sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("ov9762 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
            printk("ov9762 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	preview_setting();
#if defined(BIRD_OV9762_H_MIRROR)   
...
UINT32 OV9762MIPI_RAW_MAIN_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
