...
VOID nicCmdEventQueryLteSafeChn(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	...
	if (prCmdInfo->fgIsOid) {
		...
		/* Statistics from FW is valid */
		if (prEvent->u4Flags & BIT(0)) {
			kalMemCopy(prLteSafeChnInfo->rLteSafeChnList.au4SafeChannelBitmask,
						     prEvent->rLteSafeChn.au4SafeChannelBitmask,
						     sizeof(LTE_SAFE_CHN_INFO_T));

			DBGLOG(P2P, INFO, "[ACS]LTE safe channels= [0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x]\n",
					   prEvent->rLteSafeChn.au4SafeChannelBitmask[0],
					   prEvent->rLteSafeChn.au4SafeChannelBitmask[1],
					   prEvent->rLteSafeChn.au4SafeChannelBitmask[2],
					   prEvent->rLteSafeChn.au4SafeChannelBitmask[3],
					   prEvent->rLteSafeChn.au4SafeChannelBitmask[4]);
		}
		kalOidComplete(prGlueInfo, prCmdInfo->fgSetQuery, u4QueryInfoLen, WLAN_STATUS_SUCCESS);
	}
}
