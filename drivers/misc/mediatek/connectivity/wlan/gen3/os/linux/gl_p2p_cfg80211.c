...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		if (prMsgScanRequest == NULL) {
			DBGLOG(P2P, ERROR, "Allocate MsgScanRequest failed\n");
			i4RetRslt = -ENOMEM;
			break;
		}
		...
		prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[0] = FALSE;
#endif
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
	...
	BOOLEAN fgIsReady_2G = FALSE, fgIsReady_5G = FALSE;

	P_GLUE_INFO_T prGlueInfo = NULL;
	RF_CHANNEL_INFO_T aucChannelList_2G[MAX_2G_BAND_CHN_NUM];
	RF_CHANNEL_INFO_T aucChannelList_5G[MAX_5G_BAND_CHN_NUM];
	...
	PARAM_PREFER_CHN_INFO arChannelDirtyScore_5G[MAX_5G_BAND_CHN_NUM];
	...
	skb = cfg80211_testmode_alloc_reply_skb(wiphy, sizeof(au4AcsChnReport));
	if (!skb) {
		DBGLOG(P2P, ERROR, "Allocate skb failed\n");
		return -ENOMEM;
	}

	kalMemZero(au4AcsChnReport, sizeof(au4AcsChnReport));

	fgIsReady = prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[0];
	if (fgIsReady == FALSE)
		goto acs_report;

	fgIsReady_2G = prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[1];
	fgIsReady_5G = prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[2]
			|| prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[3]
			|| prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[4]
			|| prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[5];
	...
	if (fgIsReady_2G) {
		...
		rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_2G4, TRUE,
				     MAX_2G_BAND_CHN_NUM, &ucNumOfChannel, aucChannelList_2G);

		prGetChnLoad = &(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);

		/*
		 * 2. Calculate each 2G channel's dirty score
		 */
		for (i = 0; i < ucNumOfChannel; i++) {
			ucIdx = aucChannelList_2G[i].ucChannelNum - 1;
			...
			arChannelDirtyScore_2G[i].ucChannel = aucChannelList_2G[i].ucChannelNum;
			arChannelDirtyScore_2G[i].u2APNumScore = u2APNumScore;

			DBGLOG(P2P, INFO, "[ACS]channel=%d, AP num=%d, score=%d\n", aucChannelList_2G[i].ucChannelNum,
		...
		if (prQueryLteChn == NULL) {
			...
			/*
			 * au4SafeChannelBitmask stores LTE safe channels queried from FW.
			 * au4SafeChannelBitmask[0] indicates LTE safe channels on 2.4G, and
			 * au4SafeChannelBitmask[i] (i =1, 2, 3, 4) indicate 5G LTE safe channels
			 */
		...
		/* 4. Find out the best 2G channel, skip LTE unsafe channels */
		for (i = 0; i < ucNumOfChannel; i++) {
		...
		if (rPreferChannel.ucChannel > 0) {
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1 - 1] = BIT(31);
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1 - 1] |=
	...
	if (fgIsReady_5G) {
		/*
		* 1. Get 5G Band channel list in current regulatory domain
		*/
		rlmDomainGetChnlList(prGlueInfo->prAdapter, BAND_5G, TRUE,
				     MAX_5G_BAND_CHN_NUM, &ucNumOfChannel, aucChannelList_5G);

		prGetChnLoad = &(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);

		/*
		* 2. Calculate each 5G channel's dirty score
		*/
		for (i = 0; i < ucNumOfChannel; i++) {

			UINT_8 ucChannel = aucChannelList_5G[i].ucChannelNum;

			if (ucChannel >= 36 && ucChannel <= 48)
				ucIdx = 14 + (ucChannel - 36) / 4;
			else if (ucChannel >= 52 && ucChannel <= 64)
				ucIdx = 14 + (ucChannel - 36) / 4;
			else if (ucChannel >= 100 && ucChannel <= 144)
				ucIdx = 14 + 8 + (ucChannel - 100) / 4;
			else if (ucChannel >= 149)
				ucIdx = 14 + 8 + 12 + (ucChannel - 149) / 4;
			else {
				ucIdx = 14;
				DBGLOG(P2P, ERROR, "The channel number is illegal\n");
			}

			/* Current 5G channel's dirty score */
			u2APNumScore = prGetChnLoad->rEachChnLoad[ucIdx].u2APNum;

			arChannelDirtyScore_5G[i].ucChannel = aucChannelList_5G[i].ucChannelNum;
			arChannelDirtyScore_5G[i].u2APNumScore = u2APNumScore;

			DBGLOG(P2P, INFO, "[ACS]channel=%d, AP num=%d, score=%d\n", aucChannelList_5G[i].ucChannelNum,
			    prGetChnLoad->rEachChnLoad[ucIdx].u2APNum, u2APNumScore);
		}

		/* 3. Find out the best 5G channel */
		for (i = 0; i < ucNumOfChannel; i++) {
			if (rPreferChannel.u2APNumScore >= arChannelDirtyScore_5G[i].u2APNumScore) {
				rPreferChannel.ucChannel = arChannelDirtyScore_5G[i].ucChannel;
				rPreferChannel.u2APNumScore = arChannelDirtyScore_5G[i].u2APNumScore;
			}
		}

		if (rPreferChannel.ucChannel >= 36 && rPreferChannel.ucChannel <= 48) {
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_36 - 1] = BIT(31);
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_36 - 1] |=
				BIT((rPreferChannel.ucChannel - 36) / 4);
		} else if (rPreferChannel.ucChannel >= 52 && rPreferChannel.ucChannel <= 64) {
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_52 - 1] = BIT(31);
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_52 - 1] |=
				BIT((rPreferChannel.ucChannel - 52) / 4);
		} else if (rPreferChannel.ucChannel >= 100 && rPreferChannel.ucChannel <= 144) {
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_100 - 1] = BIT(31);
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_100 - 1] |=
				BIT((rPreferChannel.ucChannel - 100) / 4);
		} else if (rPreferChannel.ucChannel >= 149) {
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_149 - 1] = BIT(31);
			au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_149 - 1] |=
				BIT((rPreferChannel.ucChannel - 149) / 4);
		} else
			DBGLOG(P2P, INFO, "There is no prefer channel is selected\n");
	}

acs_report:
	if (unlikely(nla_put_u32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1,
		     au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_2G_BASE_1 - 1]) < 0))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_36,
		     au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_36 - 1]) < 0))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_52,
		     au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_52 - 1]) < 0))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_100,
		     au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_100 - 1]) < 0))
		goto nla_put_failure;

	if (unlikely(nla_put_u32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_149,
		     au4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_ATTR_5G_BASE_149 - 1]) < 0))
		goto nla_put_failure;

	DBGLOG(P2P, INFO, "[ACS]Relpy au4AcsChnReport[0x%08x:0x%08x:0x%08x:0x%08x:0x%08x]]\n", au4AcsChnReport[0],
		au4AcsChnReport[1], au4AcsChnReport[2], au4AcsChnReport[3], au4AcsChnReport[4]);
