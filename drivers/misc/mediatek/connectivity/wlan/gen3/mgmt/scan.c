	...
	if (prSpecificBssDesc) {
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			if (prBssDesc->ucChannelNum <= 14) {
				...
				prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[1] = TRUE;
			} else if (prBssDesc->ucChannelNum >= 36 && prBssDesc->ucChannelNum <= 48) {
				ucIdx = 14 + (prBssDesc->ucChannelNum - 36) / 4;
				prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[2] = TRUE;
			} else if (prBssDesc->ucChannelNum >= 52 && prBssDesc->ucChannelNum <= 64) {
				ucIdx = 14 + (prBssDesc->ucChannelNum - 36) / 4;
				prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[3] = TRUE;
			} else if (prBssDesc->ucChannelNum >= 100 && prBssDesc->ucChannelNum <= 144) {
				ucIdx = 14 + 8 + (prBssDesc->ucChannelNum - 100) / 4;
				prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[4] = TRUE;
			} else if (prBssDesc->ucChannelNum >= 149) {
				ucIdx = 14 + 8 + 12 + (prBssDesc->ucChannelNum - 149) / 4;
				prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[5] = TRUE;
			}
...
#if CFG_AUTO_CHANNEL_SEL_SUPPORT
		prAdapter->rWifiVar.rChnLoadInfo.fgDataReadyBit[0] = TRUE;
#endif
	}
