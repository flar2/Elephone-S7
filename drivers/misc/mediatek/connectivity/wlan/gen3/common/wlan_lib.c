...
static void wlanCfgFwSetParm(char *FwBuffer, char *cmd, char *value, int offset);
static WLAN_STATUS wlanCfgSetFw(IN P_ADAPTER_T prAdapter, const PCHAR fwBuffer, int cmdNum);
...
WLAN_STATUS wlanCfgSetFw(IN P_ADAPTER_T prAdapter, const PCHAR fwBuffer, int cmdNum)
{
	...
	memcpy(pcmdV1Header->buffer, fwBuffer, pcmdV1Header->cmdBufferLen);
...
WLAN_STATUS wlanCfgParse(IN P_ADAPTER_T prAdapter, PUINT_8 pucConfigBuf, UINT_32 u4ConfigBufLen)
{
	...
	UINT_8 FwBuffer[MAX_CMD_BUFFER_LENGTH];
	CHAR **args;
	INT_32 nargs;
	int offset = 0;
	int cmdNum = 0;
	...
	for (;;) {
		...
		case STATE_EOF:
			if (nargs > 1) {
				if (args[0][0] == '*') {
					wlanCfgFwSetParm((char *)FwBuffer, args[0], args[1], offset);
					offset += sizeof(struct _CMD_FORMAT_V1_T);
					cmdNum++;
				} else
...
exit:
	if (cmdNum)
		wlanCfgSetFw(prAdapter, (const PCHAR)FwBuffer, cmdNum);
	return WLAN_STATUS_SUCCESS;
...
void wlanCfgFwSetParm(char *FwBuffer, char *cmdstr, char *value, int offset)
{
	struct _CMD_FORMAT_V1_T cmd;
	INT_32 u4Ret = 0;
	UINT_32 valueNum = 0;

	memset(&cmd, 0, sizeof(struct _CMD_FORMAT_V1_T));
	cmd.itemType = 1;
	cmd.itemStringLength = strlen(cmdstr) - 1;
	strncpy((char *)cmd.itemString, &cmdstr[1], strlen(cmdstr) - 1);

	u4Ret = kalkStrtou32(value, 0, &valueNum);
	if (u4Ret)
		DBGLOG(INIT, ERROR, "parse au4Values error u4Ret=%d\n", u4Ret);
	memcpy(cmd.itemValue, &valueNum, sizeof(UINT_32));
	cmd.itemValueLength = strlen(cmd.itemValue);

	memcpy(FwBuffer + offset, &cmd, sizeof(struct _CMD_FORMAT_V1_T));
}
