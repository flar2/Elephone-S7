...
printk("tag1:  %s, %d \n", __func__, __LINE__);
	...
	case MSENSOR_IOCTL_READ_SENSORDATA:
		...
		xyz[0] = data.xyz.v[0]/1000;
		xyz[1] = data.xyz.v[1]/1000;
		xyz[2] = data.xyz.v[2]/1000;
		sprintf(buff, "%x %x %x %x %x", xyz[0], xyz[1], xyz[2], status, div);
...
static const struct file_operations yas537_fops = {
	.owner = THIS_MODULE,
	.open = yas537_open,
...
/*----------------------------------------------------------------------------*/
static struct miscdevice yas537_device = {
...
printk("tag1:  %s, %d \n", __func__, __LINE__);
	printk(KERN_INFO "tag1:  for set_enable is %d \n", enable );
	return count;
}
		...
		const struct i2c_device_id *id)
{
	...
	ret = misc_register(&yas537_device);
	if (ret) {
		MAG_ERR("akm537_device register failed\n");
	}
...
static int __init yas_init(void)
{
	...
	if (!hw)
		MAGN_LOG("get cust_mag dts info fail\n");
	MAGN_LOG("[%s]: i2c_number=%d,addr = 0x%x\n", __func__, hw->i2c_num,*hw->i2c_addr);
	//i2c_register_board_info(hw->i2c_num, &i2c_yas53x, 1);
	err = mag_driver_add(&yas_init_info);
        if(0 == err)
            printk("mag_driver_add OK");
	return 0;
}
