...
#ifdef CONFIG_ARM64
unsigned long ion_max_total_memory = 0xffffffff;
#endif
...
#ifdef CONFIG_ARM64
	dma_total = NODE_DATA(0)->node_zones[ZONE_DMA].managed_pages;
	/* if DMA zone is less than 2.5GB, limit the ION usage */
	if (dma_total < 0xa0000)
		ion_max_total_memory = (dma_total * 5 / 8) * 4096;
	IONMSG("ion_drv_probe ion_max_total_memory = %lu, dma_total = %lu.\n", ion_max_total_memory, dma_total);
#endif
