...
bool tp_gesture_switch=1; //jackymao
...
s32 gt1x_send_cfg(u8 * config, int cfg_len)
{
	...
	while (retry++ < 5) {
		...
		if (!ret) {
			...
			printk("Send config successfully!");
			return 0;
		}
	}
...
s32 gt1x_init_panel(void)
{
	...
	ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
	printk("gt1x_init_panel  gt1x_send_cfg ret= %d.\n",ret);
	return ret;
}
...
#define RST_RETRY       5
    ...
    do {
    	/* select i2c address */
    		// GTP_INFO("gt1x_incell_reset addr : 0x%02x", gt1x_i2c_client->addr);
        gt1x_select_addr();
...
static s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			GTP_INFO("gt1x_wakeup_level = %d",gt1x_wakeup_level);
			msleep(5);*/
