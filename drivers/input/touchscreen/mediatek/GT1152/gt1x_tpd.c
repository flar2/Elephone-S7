...
int gt1x_power_switch(s32 state)
{
	...
	static int power_state = 0;
       if(power_state ==0&&state==SWITCH_OFF)
       	{
       		return 0;
       	}
	  if(power_state ==1&&state==SWITCH_ON)
	  	{
	  		return 0;
	  	}
...
/*	
	...
	*/
	...
	/* EINT device tree, default EINT enable */
	 ...
	 ret=gt1x_i2c_read(GTP_REG_VERSION, temp_data, 2); //jackymao
     if(ret < 0)
     {
		printk(" gtp i2c error can not tpd_irq_registration\n");
	 }
	 else
   	{
...
//#endif
	...
	len=sprintf(tp_info,"TP IC:GT%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%06X,Sample FirmWare:0x%06X",gt1x_version.product_id,"HLT",client->addr,gt1x_version.patch_id,RGK_SAMPLE_VERSION);
	#else
